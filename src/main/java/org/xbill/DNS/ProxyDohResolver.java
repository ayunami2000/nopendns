 ...
 * @since 3.0
 */
public final class ProxyDohResolver implements Resolver {
    ...
    static {
        ...
        if (!System.getProperty("java.version").startsWith("1.")) {
            try {
                ...
                // fallback to Java 8
                System.out.println("Java >= 11 detected, but HttpRequest not available");
            }
        }
     ...
     */
    public ProxyDohResolver(String uriTemplate, Proxy prox) {
        this(uriTemplate, 100, Duration.ofMinutes(2), prox);
    }
     ...
     * @since 3.3
     */
    public ProxyDohResolver(
            String uriTemplate, int maxConcurrentRequests, Duration idleConnectionTimeout, Proxy prox) {
        this.proxy=prox;
        ...
        return httpClients.computeIfAbsent(
                ...
                key -> {
                    try {
                        ...
                        System.out.println("Could not create a HttpClient with for Executor " + key + " " + e);
                        return null;
                    }
                });
    }
    ...
    private static class SendAndGetMessageBytesResponse {
        int rc;
        byte[] responseBytes;
        SendAndGetMessageBytesResponse(int rca, byte[] responseBytesa){
            rc=rca;
            responseBytes=responseBytesa;
        }
    }
    ...
    private void verifyTSIG(Message query, Message response, byte[] b, TSIG tsig) {
        ...
        System.out.println("TSIG verify for query "+query.getHeader().getID()+", "+query.getQuestion().getName()+"/"+Type.string(query.getQuestion().getType())+": "+Rcode.TSIGstring(error));
    }
    ...
    public String toString() {
        return "ProxyDohResolver {" + (usePost ? "POST " : "GET ") + uriTemplate + "}";
    }
}